...
class Board {
  ...
  updateGrid() {
    ...
    for (let col = 0; col < this.grid.length; col++) {
      ...
      for (let row = 0; row < this.grid[col].length; row++) {
        ...
        // console.log(`${col}${row}: ${neighborCount}`);
        // console.log(this.grid[col][row].alive);
        if (
          (neighborCount === 2 || neighborCount === 3) &&
          this.grid[col][row].alive === true
        ) {
          // cell.alive = true;
          this.mirrorGrid[col][row].alive = true;
        } else if (neighborCount === 3 && this.grid[col][row].alive === false) {
          this.mirrorGrid[col][row].alive = true;
  ...
  drawMirrorGrid() {
    ...
    for (let col = 0; col < this.mirrorGrid.length; col++) {
      for (let row = 0; row < this.mirrorGrid[col].length; row++) {
        let square = new createjs.Shape();
        // console.log(`${x}${y}`);
        if (this.mirrorGrid[col][row].alive === false) {
