...
class Board {
  ...
  updateGrid() {
    ...
    for (let col = 0; col < this.grid.length; col++) {
      for (let row = 0; row < this.grid[col].length; row++) {
        ...
        // console.log(`${col}, ${row}: neighborCount ${neighborCount}`);
      }
    }
  ...
  drawMirrorGrid() {
    ...
    Object.keys(this.squares).forEach((key) => {
      ...
      if (this.squares[key].alive === false) {
      ...
      square.graphics.drawRect(0, 0, 5, 5);
      square.x = this.squares[key].xPos * 5;
      square.y = this.squares[key].yPos * 5;
